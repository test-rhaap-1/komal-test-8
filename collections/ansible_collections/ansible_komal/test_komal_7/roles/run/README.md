Ansible_komal.Test_komal_7 Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: ansible_komal.test_komal_7.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from ansible_komal.test_komal_7
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: ansible_komal.test_komal_7.run
      vars:
        run_x: 42
```
